...
// (`momento` del `ContextoComando`) a lo que el comando narra.
//
...
export function evento(ctx: ContextoComando, e: EventoDeComando): EventoDominio {
  return { ...e, momento: ctx.momento };
}

/** Varios de una vez, cuando el comando narra más de un hecho. */
export function eventos(ctx: ContextoComando, lista: EventoDeComando[]): EventoDominio[] {
  return lista.map((e) => evento(ctx, e));
}

/**
 * Adapta lo que devuelve una función del MOTOR que ya emite `EventoCrudo[]` (`engine/combate.ts`,
 * `engine/fusion.ts`, `engine/diplomacia.ts`) — el motor no conoce ni el momento ni a qué asentamiento
 * atribuirlo, igual que dentro del tick. Un `EventoCrudo` que todavía sea `string` plano se envuelve como
 * `'legado'`, mismo atajo que usa `simulation.ts` para un subsistema sin migrar.
 */
export function desdeCrudos(ctx: ContextoComando, crudos: EventoCrudo[], asentamientoId?: string): EventoDominio[] {
