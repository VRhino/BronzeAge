...
| Granja | Trigo | 30 madera | 4 | 6 ticks | 10 trigo |
...
**Producción de Granja duplicada** (calibración post rediseño de progreso, ver `Correcciones_Durante_Desarrollo.md` y 4.5): con una sola Granja por asentamiento y población creciendo un 12%/tick compuesto (sin techo — más población no aumenta la producción una vez cubiertos los `trabajadoresRequeridos`, ver `ratioManoObra`), la producción fija de 5 trigo/tick no alcanzaba a sostener el consumo, que crecía más rápido de lo que el disparador reactivo de una Granja nueva (solo se encola cuando la reserva proyectada cae por debajo de 5 ticks) podía compensar — el trigo llegaba a 0 sistemáticamente entre los ticks 45 y 90. Subida a 10 trigo/tick: mejora real (retrasa y suaviza la crisis) pero no la elimina del todo — sigue pendiente de una solución de fondo (ver PENDIENTE en 4.5).
...
- **Calibración** (ajustada durante implementación, sigue siendo placeholder): el coste base de madera y la velocidad de degradación se redujeron respecto a la versión inicial (que generaba espiral de déficit incluso en asentamientos bien gestionados); se subió también la velocidad de regeneración cuando el pago es íntegro. Producción base de Granja duplicada (5→10 trigo/tick, ver 4.2.1) como primera calibración del déficit de comida — mitiga pero no resuelve del todo el desajuste entre población exponencial y producción con techo fijo.
- PENDIENTE: cantidades exactas finales por nivel, en qué nivel exacto (1-3) empiezan a exigirse piedra y oro, velocidad exacta de degradación/regeneración, duración exacta del período de gracia inicial, y el tope exacto de extractores por tipo (ver 4.2 — desacoplado del nivel de asentamiento, número fijo aún sin cerrar) — todo sigue siendo ajustable, pendiente de nueva calibración por simulación tras el rediseño de progreso (el validado en pruebas de 150-300 ticks corresponde al modelo anterior). Nuevo pendiente: el desajuste de fondo entre crecimiento de población exponencial (12%/tick compuesto, sin techo de producción por Granja) sigue causando déficit de trigo en asentamientos con fertilidad baja, aunque más tarde y más suave que antes de duplicar la producción — si hace falta una solución estructural (ej. producción escalable, tope de crecimiento poblacional, disparador proactivo en vez de reactivo) queda abierto.
