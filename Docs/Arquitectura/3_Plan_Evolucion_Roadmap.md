...
- [ ] C10. Consultas derivadas servidas por el backend: la migración que [8_Triaje_Consultas.md](8_Triaje_Consultas.md) ya trió y que sigue sin empezar. **Solo dos de sus cuatro grupos** (ver el desglose en el doc 4): las **fórmulas sobre estado vivo**, que el servidor manda ya calculadas, y la **geometría por frame** (zonas, fusión, chokepoints, trazado), que viaja precalculada dentro de la proyección porque `render()` la pide en cada `mousemove`. Las de tabla las absorbe C7; `viabilidadFundacion(hover)` queda sin resolver a propósito — es una decisión de diseño de interacción
- [ ] C11. El mapa deja de ser estado: **medido 2026-08-26** — 125,4 KB idénticos byte a byte en los ticks 0, 50 y 200 (el 78% de la proyección con solo 4 facciones), viajando en cada acción desde C6. Se deriva de la seed y no cambia nunca: es un **asset**, no estado. Servirlo una vez cacheado por `seed`+`worldgenVersion` con `Cache-Control: immutable`. Incluye rasterizar el terreno, que hoy es indibujable sin `worldgen/` (`elevacion`/`fertilidad` son parámetros de ruido y el bioma no se guarda) y que es determinista por seed, así que se cachea igual
