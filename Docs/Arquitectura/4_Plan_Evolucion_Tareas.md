...
- [ ] **Pendiente**: `Membresia` sigue viviendo en memoria (se pierde al reiniciar el proceso, igual que `Sesion`); no hay endpoint para revocar ni para otorgar `moderador`/`observador`, solo se crean por unirse o por crear partida. `fundarAsentamiento` sigue fabricando ids sintéticos en vez de usar el actor real, así que un jugador no puede ganarse la ciudadanía por esa vía — con la Facción ya derivada del juego, es ahora el hueco más visible del flujo de jugador
- [ ] ~~`POST /partidas` y `POST /partidas/:gameId/tick` SIGUEN sin exigir sesión~~ — cerrado en C3 — quedan como superficie de administración sin autenticar hasta C3/C6 (separación admin/jugador, CORS, versionado); no hay auto-otorgado de `Membresia(administrador_partida)` al crear partida todavía. `Membresia.faccionId` tampoco se actualiza automáticamente tras `crearFaccion`/`comprarCasa`/`fundarAsentamiento` — la matriz lee `Membresia.faccionId` como fuente de verdad, así que hoy se queda en `null` salvo que algo externo la otorgue a mano; agravado por que `fundarAsentamiento` ni siquiera recibe el `jugadorId` real del actor (usa `numJugadores` para generar ids sintéticos, mecánica heredada de antes de la identidad real) — tensión ya señalada como abierta en el doc 5, no resuelta aquí. "Escuadrones propios del jugador o de otros residentes autorizados" (doc 5, fila de combate) tampoco se comprueba: la condición implementada solo exige residencia en el asentamiento atacante, no la propiedad de cada `escuadronId`
