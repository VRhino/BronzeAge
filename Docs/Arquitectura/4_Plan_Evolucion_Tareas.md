...
- [ ] Migrar de `GameStore` a `GameSession` las responsabilidades: cargar/crear/guardar partida, ejecutar tick y NPC, traducir errores de dominio a resultados estructurados — plan de 4 pasos en el §6 del doc 7 (crear `GameSession` → convertir `GameStore` en adaptador delgado → mover comandos por grupos → `GameStore` queda como cliente local sustituible)
- [ ] Añadir las reglas de capa de `session/` al [test de arquitectura](../../src/__tests__/arquitectura.test.ts) al crear la carpeta (`session` sin acceso a `app`/`ui`; `app` pasa a poder importar `session`)
- [ ] Mapear los **13 tipos de error de dominio** del motor a códigos estables de `ResultadoComando.codigoError` — es superficie pública de la API, hay que documentarlo una vez
