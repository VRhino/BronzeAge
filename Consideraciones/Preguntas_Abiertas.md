...
- Desajuste estructural entre población exponencial (12%/tick compuesto, sin techo salvo Vivienda) y producción de Granja con techo fijo (`ratioManoObra` no supera 1, más población no aumenta la producción): duplicar `produccionBaseTrigo` (5→10) mitiga pero no resuelve — asentamientos con fertilidad baja siguen entrando en déficit de trigo, más tarde y de forma más leve (ver `Correcciones_Durante_Desarrollo.md` #22). Posibles soluciones de fondo sin explorar todavía: producción escalable con población, tope de crecimiento poblacional, disparador proactivo de nuevas Granjas en vez de reactivo.
