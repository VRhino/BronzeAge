...
- **Revamp de caravanas** (carros, animales de arrastre, escuadrones de escolta sin héroe, cap por Mercado). **Diseño cerrado 2026-09-08** — reglas en `Docs/Game/3` §3.13, plan en `Consideraciones/Revamp_Caravanas_Definicion.md`. Planificación horaria y cría diferidas.
...
- 🔶 **Revamp de caravanas — Pasos 1-2 implementados (2026-09-08)**: la caravana `comercial` ya es un contenedor de carros (cada uno con un animal) y deriva capacidad/velocidad (`capacidadCaravana`/`velocidadCaravana`, `engine/caravanas.ts`, con fallback a `CARAVANA_CATALOGO` para las pre-revamp); casco vacío gratis (`crearCaravana`) + piezas (`agregarCarroCaravana` 20/40 madera, `comprarAnimalCaravana` buey 30 madera / caballo 60 oro / camello 40 oro); `reservadaManual` fuera del reparto automático; migración de snapshot v11→v12. **`código: ◐`** — falta el Paso 3 (estado `preparando` + lanzamiento manual) y el Paso 4 (escolta sin héroe: escuadrones cedidos por viaje, cupo por nivel de Mercado, combate y captura). Batch NPC **bit-idéntico** en los dos pasos. Descartado por medición: el bootstrap del Mercado. Reglas en `Docs/Game/3` §3.13; decisiones y plan en `Consideraciones/Revamp_Caravanas_Definicion.md`. Diferido: planificación horaria, cría, visibilidad por tamaño, inmunidad del camello al desierto.
...
- ✅ **Trazado urbano dinámico sobre rejilla** (Etapas 1-6 de `Vista_Asentamiento_Trazado_Urbano.md`): árbol único de anclas, calles como CELDAS y no aristas, conectividad como condición de validez, afinidad entre edificios del mismo tipo, afueras que crecen hacia afuera, parcelas decorativas (plaza, plaza de armas, patio de gremios, pozo, parque, puestos de mercado por nivel) — `código: ✔` (`engine/trazado.ts`, `TRAZADO`, `REJILLA_ASENTAMIENTO`, `EDIFICIO_TAMANO`)
