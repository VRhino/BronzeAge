...
- 🔶 **Revamp de caravanas — DISEÑO CERRADO (2026-09-08), sin implementar**: la caravana `comercial` pasa a ser un contenedor de **tres partes** (carros / un animal por carro / escolta de escuadrones cedidos por viaje) y deriva capacidad y velocidad; bootstrap de 1 carro + 1 buey (+50 oro) al construir el Mercado; buey / caballo / camello comprables con oro (cría diferida); carro reforzado en Carpintería; cupo de escolta por nivel de Mercado; estado `preparando` cancelable, más largo cuantos más carros; flag `reservadaManual` fuera del reparto automático. **`código: ✘`**. Lo que hay hoy: capacidad y velocidad fijas por categoría (`CARAVANA_CATALOGO`), cupo por Mercado y política, cooldown de creación. Reglas en `Docs/Game/3` §3.13; decisiones, motor y plan de 5 pasos en `Consideraciones/Revamp_Caravanas_Definicion.md`. Diferido: planificación horaria, cría, visibilidad por tamaño, inmunidad del camello al desierto.
